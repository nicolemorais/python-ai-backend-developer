...
class IteratorAccounts:
    ...
    def __next__(self):
        try:
            ...
            Account owner:\t{account.customer.name}
...
class Customer:
    ...
    def perform_transaction(self, account, transaction):
        if len(account.history.day_transactions()) >= 10:
...
class CurrentAccount(BankAccount):
    ...
    def __repr__(self):
        return f"<{self.__class__.__name__}: ('{self.branch}', '{self.number}','{self.customer.name}')>"
...
def log_transaction(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        with open(ROOT_PATH/"log.txt", "a") as file:
            file.write(
                f"[{date_time}] Function '{func.__name__}' executed with arguments {args} and {kwargs}. "
                f"Returned {result}\n"
            )
        return result
...
def list_accounts(accounts):
    for account in IteratorAccounts(accounts):
        print("=" * 100)
